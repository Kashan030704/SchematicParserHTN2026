"""Rule-based, advisory-only notes from model/SCH BOM data; never motor commands."""
...
def build_schematic_suggestions(bom):
    ...
    if not isinstance(components, list):
        if not isinstance(bom, dict) or not bom:
            return []
        components = []
        for label, quantity in bom.items():
            if not isinstance(label, str) or type(quantity) is not int or quantity < 1:
                continue
            if ":" in label:
                kind, value = label.split(":", 1)
                components.append({"type": kind, "value": value, "quantity": quantity})
            else:
                components.append({"type": label, "value": "", "quantity": quantity,
                                   "refdes": [label]})
    ...
    suggestions = [
        "These are advisory checks derived by rules from parsed BOM data, not a separate LLM review; confirm against the actual schematic nets before changing hardware."
    ]
    if any(not part["value"] for part in parts):
        suggestions.append("Some cup labels lack component values. Value-based checks are limited; labels and quantities alone cannot establish circuit safety.")
    ...
    if len(suggestions) == 1:
        suggestions.append("No additional value-based warnings were identified. This is not a circuit-safety verdict; net-level checks require schematic connectivity.")
    return suggestions
